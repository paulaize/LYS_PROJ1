"""Create a superior/inferior flipped test copy of oriented external cases.
...
only on a separate copy until visual QC confirms the result.
"""
...
DEFAULT_OUTPUT_SOURCE_NAME = "ratlesnetv2_clean_source_LSP_SI_flipped_tes"
DEFAULT_INPUT_MANIFEST_NAME = "external_dataset_manifest_LSP_oriented.csv"
DEFAULT_OUTPUT_MANIFEST_NAME = "external_dataset_manifest_LSP_SI_flipped_tes.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<external-root>/ratlesnetv2_clean_source_LSP_SI_flipped_tes"
        ),
    )
    parser.add_argument(
        "--output-manifest",
        help=(
            ...
            "<external-root>/manifests/external_dataset_manifest_LSP_SI_flipped_tes.csv"
        ),
    )
    parser.add_argument(
        "--case-id",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing outputs in the test source root",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    ...
    print(f"Wrote S/I flipped test source root: {output_source_root}")
    ...
    record.update(
        {
            ...
            "qc_flag": "needs_visual_qc_lsp_si_flip_test",
...
def _write_readme(
    ...
    *,
    ...
    lines = [
        "# LSP S/I Flipped Test External Dataset",
        "",
        ...
        "",
        "This test copy flips the voxel arrays along the selected axis while",
